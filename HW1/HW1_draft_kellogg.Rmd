...
We can take a look at the final coefficients for each variable. 
...
train_df.clean = subset(train_set, select = -c(TEAM_PITCHING_HR, TEAM_BATTING_H, TEAM_BATTING_HBP))
...
In order to select the best model to make predictions, we looked at some measurements that tells us how well each model fits. These include the 1) $R^2$, which represents the proportion of the variance explained by a model; 2) $adj R^2$, which is a modified version of $R^2$; 3) *Root Mean Squared Error* (RMSE), which is the square root of the mean squared error; and 4) *Akaike Information Criterion* (AIC), which is an estimator of out-of-sample prediction error. 

It is apparent that Model #2: Pairwise accounts for nearly 43% of the variation in the dependent variable with the independent variables, which is acceptable as a good model ($R^2 = 0.427$, $adjR^2 = 0.417$. Because the RMSE and adjusted $R^2$ statistics already include a minor adjustment for the number of coefficients estimated, to evaluate the model complexity, we compared the AIC. While a smaller AIC is deemed less complex, the difference between Model #2 and Model #3 is quite insignificant. 

```{r echo=FALSE}
# Predictions on training data
mod_sum1 = summary(lm_1)  
pred_lm1 = predict(lm_1, newdata = train_set)
...
results = data.frame(Models = c("Model #1: Backwards" , "Model #2: Pairwise", "Model #3: K-fold"),
                ...
                RSME = c(sqrt(mean((train_set$TARGET_WINS - pred_lm1)^2)), stmRMSE,
...
Using the test data and the selected final model, a comparison in the prediction statistic was conducted. 


```{r include=FALSE, eval=FALSE}
# Data Transformation to match train data
eval_set = complete(mice(data = eval_set, method = "pmm", maxit = 5, seed = 525, print = FALSE), 3)

# Add final model
predictions = predict(ADD-MODEL-HERE, newdata = eval_set)

results = cbind(dataset = c('Training Data', 'Test Prediction'), 
                bind_rows(describe(train_set$TARGET_WINS), describe(predictions))) 
rownames(results) = NULL

results[,-c(2,8,11)] %>% knitr::kable(digits = 3L, caption = 'Prediction Comparison with Final Model')
```
