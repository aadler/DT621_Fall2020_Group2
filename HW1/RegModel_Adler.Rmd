...
## Analysis
Normally, we would have thrown everything into `caret`, but for the purposes of
this homework, we will use calls to base R for the null and saturated models and
the `stepAIC` function from the `MASS` package to perform a stepwise linear
regression using AIC as our goodness of fit metric.
...
stepModel <- stepAIC(saturatedModel, scope = list(upper = ~
  (TEAM_BASERUN_CS + TEAM_BASERUN_SB) ^ 2 + (TEAM_BATTING_2B + TEAM_BATTING_3B +
   TEAM_BATTING_BB + TEAM_BATTING_H + TEAM_BATTING_SO) ^ 2 + (TEAM_FIELDING_DP +
   TEAM_FIELDING_E) ^ 2 + (TEAM_PITCHING_BB + TEAM_PITCHING_H +
   TEAM_PITCHING_HR + TEAM_PITCHING_SO) ^ 2, lower = ~ 1), direction = 'both')
...
```
The null model has an adjusted \(R^2\) of
`r prettyNum(snm$adj.r.squared, digits = 3L)` and an AIC of
`r prettyNum(AIC(nullModel), digits = 2L, big.mark = ',')`.

```{r satOutput}
knitr::kable(ssm$coefficients, digits = 3L,
             caption = 'Saturated Model Output')
```
The saturated model has an adjusted \(R^2\) of
`r prettyNum(ssm$adj.r.squared, digits = 3L)` and an AIC of
`r prettyNum(AIC(saturatedModel), digits = 2L, big.mark = ',')`.
...
The step model allowing pairwise interactions between the four classes of
metric: base running, batting, fielding, and pitching, has an adjusted \(R^2\)
of `r prettyNum(sstm$adj.r.squared, digits = 3L)` and an AIC of
`r prettyNum(AIC(stepModel), digits = 2L, big.mark = ',')`.
