...
## Analysis
Normally, we would use a statistical packahe like `caret`, which not only
...
decisions involved in predictive modeling, we will use calls to base R for the
null and saturated models and the `stepAIC` function from the `MASS` package to
perform a stepwise linear regression using AIC as our goodness of fit metric.
In the final section, we may pivot to using `caret` for final model selection.
...
### Linear Models
We will show three models: the null model, the saturated model, and the model
resulting from the stepwise regression. For the regression, we will consider at
most pairwise interactions within the classes of metrics.

```{r lm, include=FALSE}
MBsc <- dcast(MBmeltSC, INDEX ~ Metric, value.var = 'Value')
# Replace TARGET_WINS with original. Could have restored it from MBmeltUnscale.
MBsc[, TARGET_WINS := MB[, TARGET_WINS]]
# Removing INDEX for convenience. Could have used "-INDEX" in the formula call.
MBsc[, INDEX := NULL]

nullModel <- lm(TARGET_WINS ~ 1, data = MBsc)
...
saturatedModel <- lm(TARGET_WINS ~ ., data = MBsc)
...
stepModel <- stepAIC(saturatedModel, scope = list(upper = ~
...
```
The null model has an \(R^2\) of `r nmr2`, an adjusted \(R^2\) of `r nmar2` and
an AIC of `r nmAIC`.
...
```
The saturated model has an \(R^2\) of `r smr2`, an adjusted \(R^2\) of `r smar2`
and an AIC of `r smAIC`.
...
`r stmr2`, an adjusted \(R^2\) of `r stmar2` and an AIC of `r stmAIC`.

### Discussion
The first item for discussion is the observation that the intercept does **not**
change between the null (intercept-only) model and the saturated (all variables
included) model. There are 162 games in a baseball season. According to both the
null and saturated models, given no other information, a team will *lose*
slightly more than half its games. Whereas in the stepwise model allowing for
pairwise interactions within classes, given no other information, a team will
*win* slightly more than half of its games. Apparently, inteactions unearth
somewhat more information about why teams **lose** rather than why they **win**.

The stepwise model has the best \(R^2\) of the three and the best AIC. Using
accepted rules of thumb, a difference of more than 10 indicates no support for
the model with the higher AIC. The magnitude is irrelevant; it is a function of
the number of observations. The difference between the AIC of the saturated
model and the step model is
 ...
 * Curious
   ...
   * Giving up walks allows the other team free baserunners; its coefficient is
   both insignificant and near 0.
   * Giving up hits allows the other team baserunners; its coefficient is
   both insignificant and near 0.
...
`TEAM_PITCHING_HR`. Note the coefficients, batting is +10 and pitching is -7.
However, this is such as strong indicator, that in the first run, pitching was
given a *postive* coefficient of around +3. In hindsight this is because
...
terms. When giving up walks is interacted with strikeouts,
`TEAM_PITCHING_BB:TEAM_PITCHING_SO`, the coefficent is positive and significant.
Perhaps pitchers with many strikeouts also walk a lot of batters so giving up
walks on its own is less relevant. Similarly, the interaction between giving
up hits and strikeouts, `TEAM_PITCHING_H:TEAM_PITCHING_SO `, is highly negative
and significant. Once again, it probably captures most of the giving up hits
information making the singleton less relevant.

A possible explanation for the negative coefficent for getting double plays, is
that double plays require at least two people on base. That means that the
oppenent has a lot of base runners, which is very highly correlated with
scoring.
...
pairwise interactions had the lowest AIC by hundreds. 
