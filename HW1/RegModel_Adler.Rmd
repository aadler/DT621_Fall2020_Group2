...
before any normalisation or imputation, otherwise the testing set is polluted
with information from the training set. Please note that obervations prior to
...
simplifies model construction but also takes of splitting into training and
test sets and takes care of cross-validation. For the purposes of this homework,
and to demonstrate understanding of the underlying decisions involved in
predictive modeling, we will use calls to base R and the `MASS` package.

As the transforms were imposed on the entirety of the training data, we ar
...
We will show three models:
 ...
 regression, and the model
resulting from the stepwise regression. A nuFor the regression, we will consider at
most pairwise interactions within the classes of metrics.

```{r lm, include=FALSE}
MBtrnSC <- dcast(MBtrnlngSC, INDEX ~ Metric, value.var = 'Value')

# Rscale TARGET_WINS to its standard value.
...
will lose slightly more than half its games. Apparently, interctions in the
...
explaining the bizarness of the graph.

The **actual** model to be used for the prediction will be the model which
returns the best RMSE and MAPE on the test set.
 ...
 * Curious
   ...
   * Giving up hits allows the other team baserunners; why positive athough
   insignificant?
...
was given a *postive* coefficient of around +3. In hindsight this is because
...
also discussed in the previous secton.
...
### Model Evaluations
In the table below are the RMSE and MAPE of each of the three models developed
above when applied to the **testing** set.
...
nmRMSE <- sqrt(mean((MBtstSC$TARGET_WINS - nmP) ^ 2))
nmMAPE <- mean(abs((MBtstSC$TARGET_WINS - nmP) / MBtstSC$TARGET_WINS))
smRMSE <- sqrt(mean((MBtstSC$TARGET_WINS - smP) ^ 2))
smMAPE <- mean(abs((MBtstSC$TARGET_WINS - smP) / MBtstSC$TARGET_WINS))
stmRMSE <- sqrt(mean((MBtstSC$TARGET_WINS - stmP) ^ 2))
stmMAPE <- mean(abs((MBtstSC$TARGET_WINS - stmP) / MBtstSC$TARGET_WINS))
knitr::kable(data.frame(Model = c("Null", "Saturated", "Stepwise Pairwise"),
                        RMSE = c(nmRMSE, smRMSE, stmRMSE),
                        MAPE = c(nmMAPE, smMAPE, stmMAPE)),
             digits = 3L, caption = 'Test Set Results')
```

The stepwise model allowing for pairwise interactions is the winner in both
cases, as was expected.

### Predictions
We will use the stepwise-pairwise model to predict `TARGET_WINS` for the
separately provided evaluation set.
