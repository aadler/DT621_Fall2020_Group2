...
At this point we would hope to see Gaussian-looking densities.
```{r plotScaled}
...
are classic right tailed (exponential/gamma-esque) such as `TEAM_FIELDING_E` or
`TEAM_BATTING_3B`. Still others are clearly multimodal. Then there are the
weird spike-like ones as well.
...
Unfortunately, this is only defined on positive data. Therefore, the unscaled
data will need to be used. While in theory it would be nice to transform each
column as best possible, that makes for very difficult reconstruction. At this
point, we will pivot to continuing the analysis using the `caret` package which
takes care of a lot of the computational issues.
