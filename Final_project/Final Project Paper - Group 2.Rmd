...
Keeping country as a predictor is of less interest. It will most likely track
...
set.seed(181)
for (ct in unique(fullExtract$country)) {
...
stopCluster(cl)
lM1US <- linModUS$finalModel
lM1CA <- linModCA$finalModel
lM1GB <- linModGB$finalModel
lM1FR <- linModFR$finalModel
lM1DE <- linModDE$finalModel
clM1US <- linModUS$finalModel$coefficients
clM1CA <- linModCA$finalModel$coefficients
clM1GB <- linModGB$finalModel$coefficients
clM1FR <- linModFR$finalModel$coefficients
clM1DE <- linModDE$finalModel$coefficients
nclM1US <- names(clM1US)
nclM1CA <- names(clM1CA)
nclM1GB <- names(clM1GB)
nclM1FR <- names(clM1FR)
nclM1DE <- names(clM1DE)
nnclM1US <- length(nclM1US)
nnclM1CA <- length(nclM1CA)
nnclM1GB <- length(nclM1GB)
nnclM1FR <- length(nclM1FR)
nnclM1DE <- length(nclM1DE)
length(nclM1US) <- 22
length(nclM1GB) <- 22
length(nclM1FR) <- 22
length(nclM1DE) <- 22
