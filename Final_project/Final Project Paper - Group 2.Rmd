...
```
```{r loadData}
dataHeader <- "../Final_project/Data2/"
...
colClass <- c(rep('character', 4L), 'integer', 'POSIXct', 'character',
              rep('double', 4L), 'character', rep('boolean', 3L), 'character')
droppedFields <- c('description', 'thumbnail_link')

# Download, expand, and ingest raw data files
ca_set <- fread(ca_path, drop = droppedFields, colClasses = colClass)

us_set <- fread(unzip(temp), drop = droppedFields)

gb_set <- fread(unzip(temp), drop = droppedFields)

fr_set <- fread(unzip(temp), drop = droppedFields)

de_set <- fread(unzip(temp), drop = droppedFields)
...
# Convert trending_date to a date and views to double since the sum > 2^31
...
# numbers.
fullExtract <- full_set[
  full_set[, .I[which.max(trending_date)], by = c('title', 'country')]$V1
  ]
```
...
```{r addFeatures}
fullExtract[, `:=`(perfect =ifelse(likes > 0 & dislikes == 0, TRUE, FALSE),
                   lkratio = ifelse(dislikes == 0, 1, likes / dislikes))]
```

## Model Building
We will first separate 20% of the data as a true holdout set. It is on this data
that our models will be compared.

```{r holdOut}
set.seed(617)
seenIDX <- createDataPartition(fullExtract$views, p = 0.8)$Resample1
seenSet <- fullExtract[seenIDX, ]
hideSet <- fullExtract[-seenIDX, ]
```

We will train models on the remaining 80% of the data.
...
#### Model 1: Model by Country
The first linear regression model will actually be a rule-based set of models.
The hypothesis is that using country as a factor will most likely simply track
some measure of population. It is of more interest to see if there are
differences in the factors between countries. To capture this by interactions
would be combinatorially unfeasible. Instead, five models will be run, each on
a country specific subset. This is akin to a manual implementation of a rules
based regression model

```{r modelLinear1, cache = 2L}
# Using stepAIC means no cross-validation. Train on entire dataset.
trC <- trainControl(method = 'none', allowParallel = TRUE)

# Capture the unique country codes
ctCodes <- unique(fullExtract$country)
cl <- makePSOCKcluster(5)
registerDoParallel(cl)
set.seed(181)
maxPars <- 0
for (ct in ctCodes) {
  # Run the StepAIC algorithm inside the loop
  linMod <- train(views ~ likes + dislikes + comments + tags + category_id +
                    comments_disabled + ratings_disabled +
                    video_error_or_removed + perfect + lkratio,
                  data = fullExtract[country == ct],
                  family = gaussian(link = 'identity'), direction = 'both',
                  method = 'glmStepAIC', trace = 0, trControl = trC)
  
  # Assign the outcomes to dynamically "named" variables inside the loop
  assign(paste0('linMod', ct), linMod)
  assign(paste0('lM1', ct), linMod$finalModel)
  assign(paste0('clM1', ct), linMod$finalModel$coefficients)
  assign(paste0('nclM1', ct), names(linMod$finalModel$coefficients))
  assign(paste0('nnclM1', ct), length(names(linMod$finalModel$coefficients)))
  if ((length(names(linMod$finalModel$coefficients))) > maxPars) {
    maxPars <- length(names(linMod$finalModel$coefficients))
  }
}
stopCluster(cl)

# R has difficulty with ragged arrays, so override the lengths to pad with NAs
# Splitting for 80 char convenience
length(nclM1US) <- length(nclM1GB) <- length(nclM1FR) <- maxPars
length(nclM1DE) <- length(nclM1CA) <- maxPars
```

\footnotesize
```{r m1Table}
kable(data.frame(cbind(nclM1CA, nclM1DE, nclM1US, nclM1FR, nclM1GB)),
      caption = "Model Coefficients by Country")
```
\normalsize

The most parsimonious model is the one for Great Britain with only `r nnclM1GB`
parameters. The most complex is the one for Canada with `r nnclM1CA`.

### Model 2: Linear Regression Globally
While a rule-based model has the ability to fit data more closely based on those
rules, it adds a significant amount of both parameter and model error
[@Adler2015EtP; Adler2014For]. To test this hypothesis, we will run the same
linear regression, but on the global results: the sum of all the numeric values
by country except tags count. The other factors beside country deal with the
video itself, like tag count, therefore they are identical across the countries.

```{r m2Prep}
globalExtract <- unique(fullExtract[,
                                    .(views = sum(views),
                                      likes = sum(likes),
