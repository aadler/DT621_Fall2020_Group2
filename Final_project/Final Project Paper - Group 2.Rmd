...
comments using R [@RCT2020RAL]. We may also use a video’s country of origin and
...
We will add two features. The first is `disp`: the disproportion between the
likes and dislikes. To minimize division by 0 errors, it is defined as follows:
\[
disp =\begin{cases}
...
\end{cases}
\]
This constrains the ratio to \([0 \leq disp \leq 1]\) with a maximum when the
two are equal, and division by 0 only when both are 0. This usually occurs when
ratings are disabled. There is only `r ldOn` case out of the `r nobsUSExt`
observations where the ratings were not disabled, yet there are neither likes
nor dislikes. As this is a distinct incongruity for YouTube, we will remove that
one observation from the data. When ratings are disabled, perforce there is no
disparity so the ratio will be set to 1.
...
usExtract[, `:=`(disp = ifelse(rtgDisabled, 1,
...
prediction. It will then proceed through feature selection using the AIC as the
optimization metric.
...
What also seems counterintuitive at first are the negative correlations between
`lkratio` and `unan` being true with views. It may well be that conflict is good
for Youtube videos. Perhaps the more a video is talked about or argued over, the
more views it will get!
...
This model has more predictors than does the simple Gaussian (G)LM, although
their signs are similar. What stands out is how many more categories were
...
with our expectations. `Nonprofits & Activism` has the largest factor effect, to
the worse. Both `likes` and `dislikes` are positively correlated with views and
`comments` is negatively correlated. A few more categories join
`Film & Animation` as contributing to excess views over the baseline
`Entertainment`. Lastly, having a preponderance of likes tends to reduce views.
