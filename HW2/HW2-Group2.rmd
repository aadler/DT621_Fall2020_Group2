...
ROC <- function(df) {
  if (!is.data.frame(df)) {
    ...
    rocTable <- data.frame(thresh = seq(0, 1, 0.01),
                           x = double(101),
                           y = double(101))
  ...
  return(list(plot = {plot(rocTable$x, rocTable$y, type = 'l',
    ...
    lines(c(0, 1), c(0, 1), lty = 2)},
              AUC = sum((rocTable$y[1:100] + rocTable$y[2:101]) * 0.5 *
                          (rocTable$x[1:100] - rocTable$x[2:101]))))
}
```

The ROC and AUC for the dataset provided is shown below:

```{r ROCdata, echo=FALSE, fig.height=6}
ROC(class_data)
```
