...
There are `r ntrobs` observations. All of these predictors are numeric, although
some `LabelAppeal` and `STARS` could be considered ordered factors. There are a
lot of missing values for some of the predictors.

```{r missing}
Missing <- t(DT[, lapply(.SD, function(x) {sum(is.na(x))}),
                .SDcols = -c('INDEX', 'TARGET')])
kable(Missing, col.names = 'Missing',
      caption = 'Predictors with Missing Observations')
```