...
## Multiple Linear Regression 1 & Negative Binomial Model 2
### Missing Data
...
# ML1 & NB2 data transformation
# MICE imputation on train and test set
m2train <- complete(mice(data = trnSet %>% select(-INDEX),
                         method = "pmm", print = FALSE), 3)

m2test <- complete(mice(data = tstSet %>% select(-INDEX),
                        method = "pmm", print = FALSE), 3)
...
m2NBBase <- glm.nb(TARGET ~ ., data = m2train)
knitr::kable(summary(m2NBBase)$coefficients, digits = 3L,
             caption = 'Negative Binomial Regression Model 2 - Base Negative Binomial Output')
...
m2NBFit <- glm.nb(TARGET ~ VolatileAcidity + Chlorides +
                ...
                LabelAppeal + AcidIndex + STARS, data = m2train)

knitr::kable(summary(m2NBFit)$coefficients, digits = 3L,
             caption = 'Negative Binomial Regression Model 2 - Final Negative Binomial Output')
...
m1MLBase <- lm(TARGET ~ ., data = m2train)
knitr::kable(summary(m1MLBase)$coefficients, digits = 3L,
             caption = 'Multiple Regression Model 1 - Base Linear Regression Output')
...
m1MLFit <- lm(TARGET ~ VolatileAcidity + Chlorides + FreeSulfurDioxide + TotalSulfurDioxide + pH +
             Sulphates + Alcohol + LabelAppeal + AcidIndex + STARS, data = m2train)
knitr::kable(summary(m1MLFit)$coefficients, digits = 3L,
             caption = 'Multiple Regression Model 1 - Final Linear Regression Output')
...
```{r m2NBTest}
# NB2: Test Model
m2NBtstPred <- round(predict(m2NBFit, newdata = m2test))
compTable[4, 2] <- m2NBRMSE <- RMSE(m2NBtstPred, m2test$TARGET)
compTable[4, 3] <- m2NBMAE <- MAE(m2NBtstPred, m2test$TARGET)
...
```{r m1MLTest}
# ML1: Test Model
m1MLtstPred <- round(predict(m1MLFit, newdata = m2test))
compTable[5, 2] <- m1MLMSE <- RMSE(m1MLtstPred, m2test$TARGET)
compTable[5, 3] <- m1MLMAE <- MAE(m1MLtstPred, m2test$TARGET)
compTable[5, 4] <- m1MLFitAIC <- AIC(m1MLFit)

```

**Multiple Linear Regression Model 1** has an RMSE of
`r prettyNum(m1MLMSE, digits = 2L)`, an MAE of
`r prettyNum(m1MLMAE, digits = 2L)`, and a training AIC of
`r prettyNum(m1MLFitAIC, digits = 2L)`.
