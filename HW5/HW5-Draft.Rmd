...
# BUILD MODELS
## Model Set # 1
The same person is building one of each requested model. Each of the models will
follow a similar format, with only the family changing. Each model will start
with the saturated model including all the individual predictors ***except***
for `STARS`. There will also be the following interactions:
...
A forward and backwards stepwise procedure based on AIC will be used, and the
model with the lowest AIC on the training set will be the selected to be tested
against the testing set. Using stepAIC precludes the use of k-fold cross
validation.
...
m1PFit <- train(TARGET ~ . + FreeSulfurDioxide:TotalSulfurDioxide +
                  ...
                  Rated:STARS - INDEX - STARS, data = m1trnXimp,
...
magnitude effect are all positive, and they are being rated, the rating level,
and the label appeal. Keeping everything else equal, merely being rated
increases the number of cases by about \(e^{0.6509117} \approx  1.9\)!


All the coefficients are significant at at least the 5% level except for
Sulphates, but in terms of AIC, leaving it in provided a better model than
taking it out
...
It is also interesting to note that the model including `VolatileAcidity` as 
a singleton predictor has a lower deviance than the selected model, but that the
drop in deviance is not enough to offset the possible parameter error. However,
it makes itself felt through its interaction with `AcidIndex` in the selected
model.
```
                                              Df Deviance   AIC
<none>                                             9644.0 31993
+ VolatileAcidity                              1   9642.4 31994
- Sulphates                                    1   9646.5 31994
- `FreeSulfurDioxide:TotalSulfurDioxide`       1   9647.1 31994
+ pH                                           1   9643.5 31995
+ Density                                      1   9643.6 31995
+ ResidualSugar                                1   9643.6 31995
+ `VolatileAcidity:pH`                         1   9643.7 31995
+ CitricAcid                                   1   9643.9 31995
- Alcohol                                      1   9647.9 31995
+ `FixedAcidity:VolatileAcidity`               1   9643.9 31995
+ `FixedAcidity:pH:AcidIndex`                  1   9644.0 31995
+ `FixedAcidity:VolatileAcidity:pH`            1   9644.0 31995
+ `VolatileAcidity:pH:AcidIndex`               1   9644.0 31995
+ `FixedAcidity:VolatileAcidity:AcidIndex`     1   9644.0 31995
+ `FixedAcidity:pH`                            1   9644.0 31995
+ `FixedAcidity:VolatileAcidity:pH:AcidIndex`  1   9644.0 31995
- FixedAcidity                                 1   9648.1 31995
- `FixedAcidity:AcidIndex`                     1   9648.4 31996
- Chlorides                                    1   9648.9 31996
- `pH:AcidIndex`                               1   9648.9 31996
- FreeSulfurDioxide                            1   9652.3 32000
- `VolatileAcidity:AcidIndex`                  1   9661.9 32009
- TotalSulfurDioxide                           1   9662.0 32009
- AcidIndex                                    1   9678.8 32026
- LabelAppeal                                  1  10092.2 32439
- `STARS:RatedRated`                           1  10322.8 32670
- RatedRated                                   1  10325.1 32672
```
