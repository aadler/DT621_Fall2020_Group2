...
\small
```{r m1PTable}
# P1:Print results. This needed to ne in its own section so that the LaTeX
...
# then calling stepAIC on saturated model
glmc <- glm.control(maxit = 40, epsilon = 1e-8)
set.seed(72)
m1NBFitStart <- glm.nb(TARGET ~ . + FreeSulfurDioxide:TotalSulfurDioxide +
                       ...
                       data = m1trnImp, control = glmc, link = log)
