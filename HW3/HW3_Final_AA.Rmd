...
well. With the vanilla logistic regression model in R, `glm` with a binomial
family and a logit link, there are no other hyperparameters available for
tuning. More sophisticated logistic regression implementation have these
parameters for fine-tuning. There is also no reasonable way to implement
cross-validation so none will be done.
...
## Performance 
Below are the confusion matrices for the three models, followed by a table with
