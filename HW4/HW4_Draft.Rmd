...
```{r model1createDummy}
trnFreq <- tstX$TARGET_FLAG
trnSev <- tstX$TARGET_AMT
tstDumFreq <- TARGET_FLAG ~ . - c(INDEX, TARGET_AMT)
```
 ...
 * `OWN` with `HOME_VAL` as discussed above
 * `HOME_VAL` with `INCOME` as using both may overstate the effect of wealth
 * `KIDSDRIV` with `HOMEKIDS` as there is 
