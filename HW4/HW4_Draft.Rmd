...
* `HOME_VAL`  
  ...
  missing entries could indicate that the policyholder is a renter rather than a
  homeowner. Therefore, missing values were replaced with zeroes. The new `OWN`
  variables created in Model Set #1 is kept in Model Set #2.
...
* `CAR_AGE`, `INCOME` & `YOJ`  
  ...
  implausible and was assumed to be a typographical error. Thus, the absolute
  value of `CAR_AGE` is taken.
